...
class RandomPrefixSampler:
    ...
    def generate(self):
        ...
        self.pbar.start()
        threads = []
        for i in range(max(1, min(MAX_SPIDERING_THREADS, int(self.n / 5)+1))):  # 5 char-length prefix returns ~5 per search
        ...
        with open(os.path.join(ROOT_DIR, "collections", self.collection, "sample_stats.json"), "w") as f:
            json.dump({
                "collection_date": datetime.strptime(self.collection.split('_')[3], "%Y%m%d").strftime("%b %-d %Y"),
