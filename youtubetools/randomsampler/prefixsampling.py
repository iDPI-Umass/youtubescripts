...
class RandomPrefixSampler:
    ...
    def __search(self):
        while len(self.video_ids) < self.n:
            try:
                ...
                self.video_ids.extend([video_id for video_id in search_video_ids if self.is_valid_video(video_id, id_prefix)])
    ...
    def generate(self):
        self.pbar.start()
        threads = []
        for i in range(MAX_SPIDERING_THREADS):
        ...
        self.size_estimate = int((2**64)/(len(self.prefixes)*(2**self.prefix_length)*(64**(9-self.prefix_length))*16/len(self.video_ids)))
...
def get_random_prefix_sample(n=1000):
    ...
    random_prefix_sampler = RandomPrefixSampler(n)
