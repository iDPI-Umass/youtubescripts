...
def __detect_language(model: WhisperForConditionalGeneration, tokenizer: WhisperTokenizer, input_features, possible_languages: Optional[Collection[str]] = None) -> List[Dict[str, float]]:
    language_tokens = [t for t in tokenizer.additional_special_tokens if len(t) == 6]  # hacky, but all language tokens and only language tokens are 6 characters long
    ...
    logits = model(input_features, decoder_input_ids=torch.tensor([[50258] for _ in range(input_features.shape[0])])).logits  # 50258 is the token for transcribing
